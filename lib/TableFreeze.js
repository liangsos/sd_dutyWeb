...
$.fn.FrozenTable = function (iRowHead, iRowFoot, iColLeft) {
    ...
    if (iRowHead > 0 && oTable.height() > oDiv.height()) {
        ...
        oCloneDiv.css("left","0px" );//oDiv.offset().left
        oCloneDiv.css("top", "49px");//oDiv.offset().top
    }
    ...
    if (iColLeft > 0 && oTable.width() > oDiv.width()) {
        ...
        oCloneDiv.css("left", oDiv.offset().left - 201);
        oCloneDiv.css("top", oDiv.offset().top - 55);
    }
...
$.fn.CloneTable = function (oSrcTable, iRowStart, iRowEnd, iColumnEnd) {
    ...
    for (var i = iRowStart; i < iRowEnd; i++) {
        ...
        var colCount = 0;
        var colNumber = 0;
        for (var j = 0; j < (iColumnEnd == -1 ?  (i==0?oldTr.find("th").length:oldTr.find("td").length) : iColumnEnd) ; j++) {
            var oidTd = oldTr.find("td").eq(j).length == 0 ? oldTr.find("th").eq(j) : oldTr.find("td").eq(j);
            colNumber++;
